...
const SERVER_INFO = { name: 'workflow-mcp', version: '1.0.0' };

const TOOLS = [
  {
    name: 'create_workflow',
    description: '新建工作流。steps 为数组，每项 {id?, title, instruction?, dependsOn?, gate?, mode?, agentPrompt?, model?, timeoutMs?, maxRetries?, tools?, maxTurns?, outputSchema?, verify?, sandboxCwd?}；gate 是布尔表达式（支持 && || ! 和括号），只允许引用该步骤的直接依赖，依赖落定后求值为假则该步骤被自动跳过。mode=auto 的步骤可被 run_workflow 自动执行：每个 auto 步骤由服务器内 agent 循环驱动（多轮工具调用：读/写文件、列目录、grep、bash、fetch_url，全部限定在 sandboxCwd 沙箱内）；tools 为该步工具白名单，maxTurns 为循环轮数上限（默认 8），outputSchema 为结果 JSON Schema（不符自动重试一轮），verify 为完成后的质量关（JS 布尔断言或 reviewer 提示词），sandboxCwd 覆盖该步沙箱目录。顶层 finalVerify（提示词）在全部步骤落定后做整体一致性检查。创建时即做结构与环校验。',
    inputSchema: {
      type: 'object',
      properties: {
        name: { type: 'string', description: '工作流名称（目录名的一部分）' },
        ...
        steps: {
          ...
          items: {
            type: 'object',
            properties: {
              id: { type: 'string', description: '步骤唯一 id，缺省为 step-01/02…' },
              ...
              timeoutMs: { type: 'number', description: 'auto 步骤整个 agent 循环的超时毫秒数（缺省 120000）' },
              maxRetries: { type: 'number', description: 'auto 步骤失败/质量关未过后的重试次数（缺省 0，即只试 1 次）' },
              tools: { type: 'array', items: { type: 'string', enum: ['read_file', 'write_file', 'list_dir', 'grep', 'bash', 'fetch_url'] }, description: '该步允许的 agent 工具白名单；缺省全部可用（fetch_url 还需 WORKFLOW_ALLOW_NET=1）' },
              maxTurns: { type: 'number', description: 'agent 循环最大轮数（缺省 8，1-64）' },
              outputSchema: { type: 'object', description: '期望结果的 JSON Schema（如 {type:"object",required:["ok"],properties:{ok:{type:"boolean"}}}）；输出不符会带错误重试一轮，仍不符则该步失败' },
              ...
              sandboxCwd: { type: 'string', description: '该步 agent 允许访问的目录（相对服务器根目录；缺省为服务器根目录）' },
            },
            required: ['title'],
          },
        },
  ...
  {
    name: 'list_workflows',
    description: '列出全部工作流及各自进度。',
    inputSchema: { type: 'object', properties: {} },
    handler: () => store.listWorkflows(),
  },
  {
    name: 'get_workflow',
    description: '读取工作流全文。format=json 返回结构化对象（含每步状态与结果），format=md 额外返回人类可读的 WORKFLOW.md（含 mermaid 图）。',
    inputSchema: {
      type: 'object',
      properties: {
        id: { type: 'string' },
  ...
  {
    name: 'undo_step',
    description: '撤销某步骤及其全部下游（传递依赖）的完成/跳过/结果状态，用于返工。',
    inputSchema: {
      type: 'object',
  ...
  },
  {
    ...
    description: '方案B入口：返回当前可并行的步骤批次 [{stepIds, prompts, upstreamResults}]（批内并行、批间串行，按“全部成功”投影分组），宿主 agent 拿批次并行派子代理执行，每个完成后用 complete_step 回填；若中途有失败/跳过导致门条件变化，重新调用本工具取新批次。',
    inputSchema: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
